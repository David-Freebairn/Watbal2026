...
fig.update_layout(
    title=dict(
        text=(f"Monthly average: {station['name']}, "
...
with st.expander("📅 Yearly rainfall grid (monthly totals)"):
    ...
    st.download_button(
        "⬇ Download grid (CSV)",
        grid.to_csv().encode(),
        f"{station['name'].replace(' ','_')}_monthly_rain_grid.csv",
        "text/csv",
    )
...
def _build_jpeg() -> io.BytesIO:
    ...
    fig_m.suptitle(
        f"Monthly average: {station['name']}, {start_year}–{end_year}",
        fontsize=14, y=0.98,
    )
...
with d1:
    st.download_button("⬇ Monthly averages (CSV)",
                       ...
                       f"{station['name'].replace(' ','_')}_monthly_avg.csv",
                       "text/csv", use_container_width=True)
with d2:
    with st.spinner("Generating chart…"):
        jpeg_buf = _build_jpeg()
    st.download_button("🖼 Download chart (JPEG)", jpeg_buf,
                       f"{station['name'].replace(' ','_')}_monthly_avg.jpg",
