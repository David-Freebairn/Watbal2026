...
st.set_page_config(page_title="Matrix simulations", page_icon="📊", layout="wide")
...
with st.container(border=True):
    ...
    with col1:
        st.markdown("**Climate**")
        # Read from session state (set on Home page)
        ...
        if clim_source == "silo" and station:
            ...
            if not st.session_state.get(_ck):
                with st.spinner("Fetching climate..."):
                    try:
                ...
                st.caption("✅ Climate cached.")
        elif clim_source == "local":
            st.success(f"📂 {clim_label_ss}")
        else:
            st.warning("No climate selected.")
            st.page_link("app.py", label="← Go to Home to select climate")
            st.stop()

        clim_choice = None  # used only for old local path
    ...
    with col2:
        st.markdown("**Soils** — select one or more, then click ▶ Run")
        soil_selected = st.multiselect(
            "Soils",
            soil_files,
            format_func=lambda f: f.stem,
            label_visibility="collapsed",
            placeholder="Click to select soil(s)...",
        )
        ...
        vege_selected = st.multiselect(
            ...
            format_func=lambda f: f.stem,
            label_visibility="collapsed",
            placeholder="Click to select vegetation option(s)...",
        )

        n_scenarios = len(soil_selected) * len(vege_selected)
        if n_scenarios > 0:
            st.success(f"✅ Ready: **{n_scenarios} simulation(s)**  "
                       f"({len(soil_selected)} soil{'s' if len(soil_selected)>1 else ''} "
                       f"× {len(vege_selected)} vegetation)  — click ▶ Run below")
...
if run_btn:
    errors = []
    if clim_source == "silo" and not station:
        ...
        for vi, vege_path in enumerate(vege_selected):
            ...
            try:
                ...
                row = {
                    ...
                    "end"        : yr_end,
                    "nyears"     : nyears,
                    "rainfall"   : int(round(float(ann["rain"].mean()))),
                    "runoff"     : int(round(float(ann["runoff"].mean()))),
                    "soil_evap"  : int(round(float(ann["soil_evap"].mean()))),
                    "transp"     : int(round(float(ann["transp"].mean()))),
                    "et"         : int(round(float(ann["et"].mean()))),
                    "drainage"   : int(round(float(ann["drainage"].mean()))),
                    "irrigation" : 0,
                    "err_yr"     : round(err, 4),
                }
...
if saved:
    ...
    # Round mm columns to integers BEFORE rename (cleaner column names at this point)
    for _c in ["rainfall","irrigation","soil_evap","transp","runoff","drainage","et"]:
        if _c in df_show.columns:
            df_show[_c] = pd.to_numeric(df_show[_c], errors="coerce").fillna(0).round(0).astype(int)
    for _c in ["erosion_t_ha"]:
        if _c in df_show.columns:
            df_show[_c] = pd.to_numeric(df_show[_c], errors="coerce").fillna(0).round(1)
    for _c in ["yield_t_ha"]:
        if _c in df_show.columns:
            df_show[_c] = pd.to_numeric(df_show[_c], errors="coerce").fillna(0).round(2)
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Soil Type":        st.column_config.TextColumn(width="medium"),
            "Veg Option":       st.column_config.TextColumn(width="medium"),
            "Rainfall\n(mm)":  st.column_config.NumberColumn(format="%d"),
            "Irrigation\n(mm)":st.column_config.NumberColumn(format="%d"),
            "Runoff\n(mm)":    st.column_config.NumberColumn(format="%d"),
            "Soil Evap\n(mm)": st.column_config.NumberColumn(format="%d"),
            "Transp.\n(mm)":   st.column_config.NumberColumn(format="%d"),
            "Drainage\n(mm)":  st.column_config.NumberColumn(format="%d"),
            "Erosion\n(t/ha)": st.column_config.NumberColumn(format="%.1f"),
